...
FROM parquet_full_metadata(%s)
	...
	for (auto &row : *result) {
		...
		auto &file_metadata_list_entries = ListVector::GetChild(file_metadata_vec);
		auto &parquet_metadata_list_entries = ListVector::GetChild(parquet_metadata_vec);
		auto &parquet_schema_list_entries = ListVector::GetChild(parquet_schema_vec);
