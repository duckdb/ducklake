...
static optional_ptr<MultiFileColumnDefinition>
TryFindColumnByFieldId(const vector<MultiFileColumnDefinition> &local_columns, int32_t field_id,
...
MultiFileReaderVirtualColumnBinding DuckLakeMultiFileReader::GetVirtualColumnExpression(
	...
	if (column_id == COLUMN_IDENTIFIER_ROW_ID) {
		...
		auto found_column =
		    TryFindColumnByFieldId(local_columns, MultiFileReader::ROW_ID_FIELD_ID, row_id_column.get());
		if (found_column) {
			return MultiFileReaderVirtualColumnBinding(*found_column);
		}
	...
	if (column_id == COLUMN_IDENTIFIER_SNAPSHOT_ID) {
		auto found_column = TryFindColumnByFieldId(local_columns, MultiFileReader::LAST_UPDATED_SEQUENCE_NUMBER_ID,
		                                           snapshot_id_column.get());
		if (found_column) {
			return MultiFileReaderVirtualColumnBinding(*found_column);
		}
		// get the row id start for this file
