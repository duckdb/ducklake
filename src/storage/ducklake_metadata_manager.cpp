...
string DuckLakeMetadataManager::GenerateConstantFilter(ExpressionType comparison_type, const Value &constant,
	...
	if (filter_sql && !filter_sql->can_compare(constant, type)) {
		return string();
	}
...
DuckLakeMetadataManager::GenerateCTESectionFromRequirements(const unordered_map<idx_t, CTERequirement> &requirements,
                                                            TableIndex table_id,
                                                            FileColumnStatsCTEBodyGenerator body_generator) {
	...
	for (const auto &entry : requirements) {
		...
		cte_section += body_generator ? body_generator(req, table_id) : GenerateFileColumnStatsCTEBody(req, table_id);
		cte_section += ")";
	}
...
void DuckLakeMetadataManager::MergeColumnStatsRequirement(unordered_map<idx_t, CTERequirement> &requirements,
                                                          const CTERequirement &new_requirement) {
	auto entry = requirements.find(new_requirement.column_field_index);
	if (entry == requirements.end()) {
		requirements.emplace(new_requirement.column_field_index, new_requirement);
		return;
	}
	entry->second.referenced_stats.insert(new_requirement.referenced_stats.begin(),
	                                      new_requirement.referenced_stats.end());
	entry->second.reference_count += new_requirement.reference_count;
}
...
DuckLakeMetadataManager::GetBucketPartitionPruningConstraints(DuckLakeTableEntry &table,
	...
	vector<DuckLakeFileListBucketConstraint> result;
	...
	for (auto &field : partition_data->fields) {
		...
		result.push_back({field.partition_key_index, std::move(bucket_values)});
	}
	return result;
}

string DuckLakeMetadataManager::GenerateBucketPartitionPruningClause(
    const vector<DuckLakeFileListBucketConstraint> &constraints, TableIndex table_id,
    const string &partition_value_table) {
	string result;
	for (const auto &constraint : constraints) {
		string in_list;
		for (const auto &value : constraint.partition_values) {
			if (!in_list.empty()) {
				in_list += ", ";
			}
			in_list += SQLString(value);
		}
		if (!result.empty()) {
			result += " AND ";
		}
		result += StringUtil::Format(
		    "data.data_file_id IN (SELECT data_file_id FROM %s WHERE table_id = %d AND partition_key_index = %d "
		    "AND partition_value IN (%s))",
		    partition_value_table, table_id.index, constraint.partition_key_index, in_list);
	}
	return result;
}
...
string DuckLakeMetadataManager::BuildBucketPartitionPruningClause(DuckLakeTableEntry &table,
                                                                  const FilterPushdownInfo &filter_info) {
	return GenerateBucketPartitionPruningClause(GetBucketPartitionPruningConstraints(table, filter_info),
	                                            table.GetTableId(), "{METADATA_CATALOG}.ducklake_file_partition_value");
}

string DuckLakeMetadataManager::GenerateFileListQuery(const DuckLakeFileListOperation &operation) {
	FilterSQLResult filter_result;
	if (operation.filter_info && !operation.filter_info->column_filters.empty()) {
		filter_result = ConvertFilterPushdownToSQL(*operation.filter_info);
	}

	auto bucket_clause = GenerateBucketPartitionPruningClause(operation.bucket_constraints, operation.table_id,
	                                                          "{METADATA_CATALOG}.ducklake_file_partition_value");
	...
	for (const auto &dynamic_filter : operation.dynamic_filters) {
		MergeColumnStatsRequirement(filter_result.required_ctes,
		                            CTERequirement(dynamic_filter.column_field_index, {"min_value", "max_value"}));
	}

	string query = GenerateCTESectionFromRequirements(filter_result.required_ctes, operation.table_id);
	...
	for (const auto &dynamic_filter : operation.dynamic_filters) {
		...
		if (order_by_clause.empty()) {
			...
			if (seeking_high_values) {
				auto cast_expr = CastStatsToTarget(cte_name + ".max_value", dynamic_filter.column_type);
				order_by_clause = StringUtil::Format("\nORDER BY %s DESC NULLS LAST", cast_expr);
			} else if (seeking_low_values) {
				auto cast_expr = CastStatsToTarget(cte_name + ".min_value", dynamic_filter.column_type);
...
WHERE data.table_id=%d AND {SNAPSHOT_ID} >= data.begin_snapshot AND ({SNAPSHOT_ID} < data.end_snapshot OR data.end_snapshot IS NULL)
	                            ...
	                            select_list, stats_join_list, operation.table_id.index, operation.table_id.index);
	if (!filter_result.where_conditions.empty()) {
...
vector<DuckLakeFileListEntry> DuckLakeMetadataManager::GetFilesForTable(DuckLakeTableEntry &table,
	...
	if (filter_info) {
		...
		operation.bucket_constraints = GetBucketPartitionPruningConstraints(table, *filter_info);
	}

	auto query = GenerateFileListQuery(operation);
	auto result = Query(operation.snapshot, query);
	if (result->HasError()) {
