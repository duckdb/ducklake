...
CREATE INDEX idx_ducklake_file_col_stats_tbl_col ON {METADATA_CATALOG}.ducklake_file_column_stats (table_id, column_id);
INSERT INTO {METADATA_CATALOG}.ducklake_metadata (key, value) VALUES ('version', '0.5'), ('created_by', 'DuckDB %s'), ('data_path', %s), ('encrypted', '%s');
...
void DuckLakeMetadataManager::MigrateV04(bool allow_failures) {
	string migrate_query = R"(
CREATE INDEX {IF_NOT_EXISTS} idx_ducklake_file_col_stats_tbl_col ON {METADATA_CATALOG}.ducklake_file_column_stats (table_id, column_id);
UPDATE {METADATA_CATALOG}.ducklake_metadata SET value = '0.5' WHERE key = 'version';
	)";
	ExecuteMigration(migrate_query, allow_failures, "0.4", "0.5");
}
